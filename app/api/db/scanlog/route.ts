...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const ScanlogResponse = await axios.post(`${process.env.DESKTOP_URL}/scanlog/new`, dataSn, {
      headers: {
    ...
    if (ScanlogData.length === 0) {
      return NextResponse.json({ success: true, message: 'No scanlog to create' });
    }
    ...
    const createdScanlog = await scanLog.bulkCreate(DataMap, { transaction, returning: true });
    
    await transaction?.commit();

    return NextResponse.json({ success: true, message: 'New Scanlog created successfully' });
